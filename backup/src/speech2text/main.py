...
def init():
    ...
    st.subheader(
        "You want to extract text from an audio/video? You are in the right place!")
...
def transcribe_audio_part(filename, stt_model, stt_tokenizer, myaudio, sub_start,
    ...
    try:
        with torch.no_grad():
            ...
            path = filename.stem[:-3] + "audio_" + str(index) + ".mp3"
            ...
            input_values = stt_tokenizer(input_audio, return_tensors="pt").to(
                device).input_values
...
def detect_silences(audio):
    ...
    silence_list = silence.detect_silence(audio, min_silence_len=750,
...
def silences_distribution(silence_list, min_space, max_space, start, end,
                          srt_token=False):
    ...
    if silence_list[0] - max_space > newsilence[0]:
        for i in range(int(newsilence[0]), int(silence_list[0]),
                       max_space):  # int bc float can't be in a range loop
            value = i + max_space
    ...
    while nb_values != 0:
        ...
        if silence_window != []:
            ...
            if srt_token:
                nearest_value = min(silence_window,
                                    key=lambda x: abs(x - min_desired_value))
                nb_values -= silence_window.index(
                    nearest_value) + 1  # (index begins at 0, so we add 1)
            else:
                nearest_value = min(silence_window,
                                    key=lambda x: abs(x - max_desired_value))
    ...
    if end - newsilence[-1] > min_space:
        ...
        if end - newsilence[-1] < max_space:
            ...
            newsilence = generate_regular_split_till_end(newsilence, end, min_space,
                                                         max_space)
    else:
...
def transcription_non_diarization(filename, myaudio, start, end, srt_token, stt_model,
                                  stt_tokenizer, min_space, max_space, save_result,
                                  txt_text, srt_text):
    # get silences
    silence_list = detect_silences(myaudio)
    if silence_list != []:
        silence_list = get_middle_silence_time(silence_list)
        silence_list = silences_distribution(silence_list, min_space, max_space, start,
                                             end, srt_token)
    else:
        silence_list = generate_regular_split_till_end(silence_list, int(end),
                                                       min_space, max_space)
    ...
    for i in range(0, len(silence_list) - 1):
        ...
        transcription = transcribe_audio_part(filename, stt_model, stt_tokenizer,
                                              myaudio, sub_start, sub_end, i)

        if transcription != "":
            save_result, txt_text, srt_text = display_transcription(transcription,
                                                                    save_result,
                                                                    txt_text, srt_text,
                                                                    sub_start, sub_end)
        ...
        if transcript_btn:
            ...
            with st.spinner("We are transcribing your audio. Please wait"):
                ...
                save_result, txt_text, srt_text = transcription_non_diarization(
                    filename, myaudio, start, end, srt_token, stt_model, stt_tokenizer,
                    min_space, max_space, save_result, txt_text, srt_text)

                # Delete files
                # clean_directory(
                #     "../../data")  # clean folder that contains generated files
                ...
                if txt_text != "":
                    ...
                    st.write(
                        "Transcription impossible, a problem occurred with your audio or your parameters, we apologize :(")
...
def transcript_from_file(tokenizer, model):
    uploaded_file = st.file_uploader("Upload your file! It can be a .mp3, .mp4 or .wav",
                                     type=["mp3", "mp4", "wav"])

    if uploaded_file is not None:
        # get name and launch transcription function
        filename = uploaded_file.name
        transcription(tokenizer, model, filename, uploaded_file)
    else:
        print('_')


if __name__ == '__main__':
    stt_tokenizer, stt_model = init()
    choice = st.radio("Features", ["By uploading a file"])
